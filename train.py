...
for epoch in range(num_epochs_stage2):
    for batch in dataloader:
        ...
        for frame_idx in range(video_frames.shape[1]):
            ...
            # Total loss
            total_loss = lip_sync_loss + vgg_loss_frame + identity_loss_value
        ...
        print(f"Stage 2 - Epoch [{epoch+1}/{num_epochs_stage2}], Total Loss: {total_loss.item()}, Lip Sync Loss: {lip_sync_loss.item()}, Perceptual Loss: {vgg_loss_frame.item()}, Identity Loss: {identity_loss_value.item()}")
