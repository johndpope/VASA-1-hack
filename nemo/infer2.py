...
def log_tensor_state(name: str, tensor: Optional[torch.Tensor], detailed: bool = False) -> None:
...
#         self.graph = SegmentationWrapper()
    ...
    def forward(self,
        ...
        """
        """Forward pass of the inference wrapper."""
        log_processing_step("Starting Forward Pass")
        ...
        logger.debug(f"  target_theta: {target_theta}")
        logger.debug(f"  mix: {mix}")
        logger.debug(f"  mix_old: {mix_old}")
        logger.debug(f"  no_grad_infer: {no_grad_infer}")

        # Process source image if provided
        if source_image is not None:
            logger.debug("\nProcessing source image input")
            if isinstance(source_image, PIL.Image.Image):
                logger.debug(f"Source image size: {source_image.size}")
            ...
            if source_result is None:
                logger.error("Source image processing failed")
                return None, None

        # Process driver image if provided
        if driver_image is not None:
            logger.debug("\nProcessing driver image input")
            if isinstance(driver_image, list):
                logger.debug(f"Processing batch of {len(driver_image)} driver images")
                if len(driver_image) > 0 and isinstance(driver_image[0], PIL.Image.Image):
                    logger.debug(f"First driver image size: {driver_image[0].size}")
            elif isinstance(driver_image, PIL.Image.Image):
                logger.debug(f"Single driver image size: {driver_image.size}")
    ...
    def _process_source_features(self, source_img_crop: torch.Tensor,
        ...
        try:
            log_processing_step("Processing Source Features")
            
            # Log input states
            log_tensor_state("Input source_img_crop", source_img_crop)
            log_tensor_state("Input source_img_mask", source_img_mask)
            
            # Get model dimensions
            c = self.args.latent_volume_channels
            s = self.args.latent_volume_size
            d = self.args.latent_volume_depth
            logger.debug(f"Volume dimensions: c={c}, s={s}, d={d}")
            
            # Generate embeddings
            logger.debug("\nGenerating identity embedding:")
            masked_input = source_img_crop * source_img_mask
            log_tensor_state("Masked input", masked_input)
            
            self.idt_embed = self.model.idt_embedder_nw.forward_image(masked_input)
            log_tensor_state("Identity embedding", self.idt_embed)
            
            # Generate source latents
            logger.debug("\nGenerating source latents:")
            source_latents = self.model.local_encoder_nw(masked_input)
            log_tensor_state("Source latents", source_latents)
            
            # Process source pose
            logger.debug("\nComputing source pose:")
            with torch.no_grad():
            ...
            log_tensor_state("Initial grid", grid)
            ...
            log_data_dict("Initial", data_dict)
            ...
            log_tensor_state("Source pose embed", source_pose_embed)
            ...
            log_data_dict("Warp embed dict", source_warp_embed_dict)
            
            xy_gen_outputs = self.model.xy_generator_nw(source_warp_embed_dict)
            log_tensor_state("XY generator output", xy_gen_outputs[0])
            ...
            if self.resize_warp:
                source_xy_warp_resize = self.model.resize_warp_func(source_xy_warp_resize)
                log_tensor_state("Resized source XY warp", source_xy_warp_resize)
            
            # Process volumes
            logger.debug("\nProcessing volumes:")
            source_latents_face = source_latents
            source_latent_volume = source_latents_face.view(1, c, d, s, s)
            log_tensor_state("Initial source volume", source_latent_volume)
            
            if self.args.source_volume_num_blocks > 0:
                source_latent_volume = self.model.volume_source_nw(source_latent_volume)
                log_tensor_state("Processed source volume", source_latent_volume)
            ...
            logger.debug("\nSource feature processing complete")
            return True
                
        except Exception as e:
            logger.error(f"Error in source feature processing: {str(e)}")
    ...
    def _generate_target_features(self, driver_img_crop: torch.Tensor,
        ...
        try:
            # Log initial state
            log_processing_step("Generating Target Features")
            logger.debug("Initial state:")
            log_tensor_state("driver_img_crop", driver_img_crop)
            log_tensor_state("pred_target_theta", pred_target_theta)
            log_tensor_state("driver_mask", driver_mask)
            log_tensor_state("custome_target_pose_embed", custome_target_pose_embed)
            log_tensor_state("idt_embed", self.idt_embed)

            # Setup transformation grid
            logger.debug("\nSetting up transformation grid:")
            c = self.args.latent_volume_channels
            s = self.args.latent_volume_size
            d = self.args.latent_volume_depth
            logger.debug(f"Volume parameters: c={c}, s={s}, d={d}")

            grid = self.model.identity_grid_3d.repeat_interleave(1, dim=0)
            log_tensor_state("Initial grid", grid)

            # Apply target theta transformation
            logger.debug("\nApplying target theta transformation:")
            if self.target_theta:
                logger.debug("Using target theta")
                target_theta_part = pred_target_theta[:, :3].transpose(1, 2)
                target_rotation_warp = grid.bmm(target_theta_part)
                target_rotation_warp = target_rotation_warp.view(-1, d, s, s, 3)
            else:
                logger.debug("Using source theta")
                source_theta_part = self.pred_source_theta[:, :3].transpose(1, 2)
                target_rotation_warp = grid.bmm(source_theta_part)
                target_rotation_warp = target_rotation_warp.view(-1, d, s, s, 3)
            log_tensor_state("Target rotation warp", target_rotation_warp)

            # Process driver mask
            logger.debug("\nProcessing driver mask:")
            if driver_mask is not None:
                logger.debug("Using provided mask")
                driver_img_mask = driver_mask
            else:
                logger.debug("Generating new mask")
                driver_img_mask = self.get_mask(driver_img_crop)
            driver_img_mask = driver_img_mask.to(driver_img_crop.device)
            log_tensor_state("Driver mask", driver_img_mask, detailed=True)
            ...
            logger.debug("\nProcessing expression embedding:")
            data_dict = self.model.expression_embedder_nw(data_dict, True, False)
            log_data_dict("After expression embedder", data_dict)

            # Handle custom pose embedding
            if custome_target_pose_embed is not None:
                logger.debug("Using custom pose embedding")
                data_dict['target_pose_embed'] = custome_target_pose_embed

            # Store pose embed and aligned image
            self.target_pose_embed = data_dict['target_pose_embed']
            self.target_img_align = data_dict.get('target_img_align')
            log_tensor_state("Target pose embed", self.target_pose_embed)
            log_tensor_state("Target img align", self.target_img_align)

            # Generate target features
            logger.debug("\nGenerating target features and warping:")
            _, target_warp_embed_dict, _, embed_dict = self.model.predict_embed(data_dict)
            log_data_dict("Target warp embed dict", target_warp_embed_dict)

            target_uv_warp, data_dict['target_delta_uv'] = self.model.uv_generator_nw(target_warp_embed_dict)
            log_tensor_state("UV warp", target_uv_warp)
            log_tensor_state("Delta UV", data_dict['target_delta_uv'])

            # Process warping
            target_uv_warp_resize = target_uv_warp
            if self.resize_warp:
                logger.debug("Applying warp resize")
                target_uv_warp_resize = self.model.resize_warp_func(target_uv_warp_resize)
                log_tensor_state("Resized UV warp", target_uv_warp_resize)

            # Generate aligned target volume
            logger.debug("\nGenerating aligned target volume:")
            logger.debug("Current volume states:")
            log_tensor_state("Target latent volume", self.target_latent_volume)
            log_tensor_state("Target UV warp resize", target_uv_warp_resize)
            log_tensor_state("Target rotation warp", target_rotation_warp)

            temp_volume = self.model.grid_sample(self.target_latent_volume, target_uv_warp_resize)
            log_tensor_state("After first grid sample", temp_volume)

            aligned_target_volume = self.model.grid_sample(temp_volume, target_rotation_warp)
            ...
            img, *outputs = self.model.decoder_nw(
                data_dict,
                embed_dict,
                target_latent_feats,
                False,
            ...
            log_tensor_state("Generated image", img, detailed=True)

            logger.debug("Target feature generation complete")
            return img

        except Exception as e:
            logger.error(f"Error in target feature generation: {str(e)}")
            logger.error(traceback.format_exc())
            logger.error("\nTensor states at error:")
            for name, tensor in locals().items():
                if isinstance(tensor, torch.Tensor):
                    log_tensor_state(f"  {name}", tensor, detailed=True)
            raise
    ...
    def _process_driver_image(self, driver_image: Union[PIL.Image.Image, List[PIL.Image.Image]],
        ...
        """
        """Process driver image with detailed logging."""
        log_processing_step("Driver Image Processing")
        
        # Log input parameters
        logger.debug("\nInput parameters:")
        logger.debug(f"  crop: {crop}")
        logger.debug(f"  smooth_pose: {smooth_pose}")
        log_tensor_state("driver_mask", driver_mask)
        log_tensor_state("custome_target_pose_embed", custome_target_pose_embed)
        log_tensor_state("custome_target_theta_embed", custome_target_theta_embed)

        context = torch.no_grad() if self.no_grad_infer else contextlib.nullcontext()
        logger.debug(f"Using no_grad context: {self.no_grad_infer}")
        
        with context:
            # Prepare driver image
            logger.debug("\nPreparing driver image:")
            ...
            logger.debug(f"Moved driver image to device: {self.device}")
            
            # Process driver pose
            logger.debug("\nProcessing driver pose:")
            pred_target_theta = self._process_driver_pose(
                driver_img_crop, smooth_pose, custome_target_theta_embed)
            log_tensor_state("Predicted target theta", pred_target_theta)
            ...
            logger.debug(f"Generated {len(pred_target_img)} output images")
    ...
    def _prepare_driver_images(self, driver_image: Union[PIL.Image.Image, List[PIL.Image.Image]],
        ...
        """
        log_processing_step("Driver Image Preparation")
        
        if crop:
            logger.debug("Applying face crop")
            if not isinstance(driver_image, list):
                logger.debug("Converting single image to list")
                driver_image = [driver_image]
                
            # Detect faces
            logger.debug("\nDetecting faces:")
            driver_faces = [self._detect_face(img) for img in driver_image]
            logger.debug(f"Detected {len([f for f in driver_faces if f is not None])} faces in {len(driver_faces)} images")
            
            # Convert to tensors
            logger.debug("\nConverting to tensors:")
            driver_tensors = [self.to_tensor(img) for img in driver_image]
            for i, tensor in enumerate(driver_tensors):
                log_tensor_state(f"Driver tensor {i}", tensor)
            
            # Crop faces
            logger.debug("\nCropping faces:")
            driver_img_crop, face_check, face_scale_stats = self.crop_image(
                driver_tensors, driver_faces)
            logger.debug(f"Face detection success rate: {face_check.mean():.2f}")
            logger.debug(f"Average face scale: {np.mean(face_scale_stats):.3f}")
            log_tensor_state("Cropped result", driver_img_crop)
            
        else:
            logger.debug("Skipping face crop")
            if not isinstance(driver_image, list):
                logger.debug("Processing single image")
                driver_image = self.convert_to_tensor(driver_image)[:, :3]
                log_tensor_state("Converted tensor", driver_image)
            ...
            log_tensor_state("Resized image", driver_img_crop)
    ...
    def _process_driver_pose(self, driver_img_crop: torch.Tensor,
        ...
        """
        """Process driver pose with detailed logging."""
        log_processing_step("Driver Pose Processing")
        
        logger.debug("\nInput state:")
        log_tensor_state("Driver image crop", driver_img_crop)
        log_tensor_state("Custom theta embed", custome_target_theta_embed)
        
        # Get initial pose
        with torch.no_grad():
            logger.debug("\nComputing initial pose:")
            pred_target_theta, scale, rotation, translation = self.model.head_pose_regressor.forward(
                driver_img_crop, return_srt=True)
            
            logger.debug("Pose components:")
            log_tensor_state("Predicted theta", pred_target_theta)
            log_tensor_state("Scale", scale)
            log_tensor_state("Rotation", rotation)
            log_tensor_state("Translation", translation)
        ...
        if custome_target_theta_embed is not None:
            logger.debug("\nUsing custom theta embedding")
            pred_target_theta = point_transforms.get_transform_matrix(*custome_target_theta_embed)
            log_tensor_state("Custom theta matrix", pred_target_theta)
        
        # Apply mixing if enabled
        if self.mix:
            logger.debug("\nApplying theta mixing:")
            log_tensor_state("Source theta", self.pred_source_theta)
            pred_target_theta = self.get_mixing_theta(self.pred_source_theta, pred_target_theta)
            log_tensor_state("Mixed theta", pred_target_theta)
        
        # Apply pose smoothing
        if smooth_pose:
            logger.debug("\nApplying pose smoothing:")
            pred_target_theta = self._smooth_pose(pred_target_theta)
            log_tensor_state("Smoothed theta", pred_target_theta)
    ...
    def _smooth_pose(self, pred_target_theta: torch.Tensor) -> torch.Tensor:
        ...
        """
        """Apply pose smoothing with detailed logging."""
        log_processing_step("Pose Smoothing")
        
        logger.debug("\nInput state:")
        log_tensor_state("Target theta", pred_target_theta)
        log_tensor_state("Current theta", self.theta if self.theta is not None else None)
        logger.debug(f"Momentum: {self.pose_momentum}")
        
        # Initialize theta if needed
        if self.theta is None:
            logger.debug("Initializing theta")
            self.theta = pred_target_theta[0].clone()
            log_tensor_state("Initial theta", self.theta)
        ...
        # Stack results
        result = torch.stack(smooth_driver_theta)
        log_tensor_state("Smoothed result", result)
    ...
    def process_video(self, 
        ...
        """Process video frames with detailed logging."""
        log_processing_step("Video Processing")
        
        # Log initial parameters
        logger.debug("\nInitial parameters:")
        logger.debug(f"Video path: {video_path}")
        logger.debug(f"Max frames: {max_frames}")
        if isinstance(source_img, PIL.Image.Image):
            logger.debug(f"Source image size: {source_img.size}")
        ...
        while True:
            ...
            if len(driving_frames) % 100 == 0:
                logger.debug(f"Loaded {len(driving_frames)} frames")
        
        cap.release()
        logger.info(f"Loaded {len(driving_frames)} frames total")
        ...
        generated_frames = []
        if results:
            logger.debug("Successfully generated first frame")
            generated_frames.append(results[0][0])
        else:
            logger.error("Failed to generate first frame")
            return [], []
        ...
        for i, frame in enumerate(driving_frames[1:], 1):
            ...
            logger.debug(f"\nProcessing frame {i}/{len(driving_frames)}")
            ...
            try:
                ...
                if results:
                    generated_frames.append(results[0][0])
                    logger.debug(f"Generated frame {i} successfully")
                else:
                    logger.error(f"Failed to generate frame {i}")
                ...
                if i % 10 == 0:
                    ...
                    logger.info(f"Progress: {i}/{len(driving_frames)} frames | "
        ...
        logger.info("\nProcessing complete:")
        logger.info(f"Total frames processed: {len(driving_frames)}")
        logger.info(f"Total time: {total_time:.2f}s")
        logger.info(f"Average FPS: {avg_fps:.2f}")
        logger.info(f"Average time per frame: {1000 * total_time / len(driving_frames):.2f}ms")
        logger.info(f"Successfully generated frames: {len(generated_frames)}")
        
        if len(generated_frames) != len(driving_frames):
            logger.warning(f"Missing {len(driving_frames) - len(generated_frames)} frames")
...
inferer = InferenceWrapper(experiment_name = 'speedup', model_file_name = '328_model.pth',
                           project_dir = '.', folder = 'logs', state_dict = None,
