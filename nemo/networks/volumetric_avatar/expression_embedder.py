...
class ExpressionEmbed(nn.Module):
    ...
    def forward(self, data_dict, estimate_kp_by_net = False, use_seg=False, use_aug = True):
        logger.debug("\n=== ExpressionEmbed Forward Pass ===")
        logger.debug("\nInput tensors:")
        for k, v in data_dict.items():
            if isinstance(v, torch.Tensor):
                logger.debug(f"  {k}: shape={v.shape}, device={v.device}")
        ...
        logger.debug(f"\nConcatenated inputs shape: {inputs.shape}")

        n = data_dict['source_img'].shape[0]
        t = data_dict['target_img'].shape[0]
        logger.debug(f"Source batch size (n): {n}, target batch size (t): {t}")
        ...
        if estimate_kp_by_net:
            ...
            logger.debug(f"\nKP net theta shape: {theta.shape}")
        else:
            scale, rotation, translation = self.net_head(inputs)[0]
            logger.debug(f"\nHead network outputs:")
            logger.debug(f"  scale: {scale.shape}")
            logger.debug(f"  rotation: {rotation.shape}")
            logger.debug(f"  translation: {translation.shape}")
            
            theta = get_similarity_transform_matrix(scale, rotation, translation)
            logger.debug(f"Computed theta shape: {theta.shape}")

            data_dict['pred_source_theta'] = theta[:n]
            data_dict['pred_target_theta'] = theta[-t:]

        if self.training:
            logger.debug("\nTraining mode processing:")
            ...
            if not estimate_kp_by_net:
                ...
                logger.debug(f"Estimated theta shape: {theta.shape}")
            else:
                theta = torch.cat([data_dict['source_theta'][:,:3,:], data_dict['target_theta'][:,:3,:]])
                logger.debug(f"KP net training theta shape: {theta.shape}")
            ...
            if use_seg:
                ...
                inputs_face = torch.cat([data_dict['source_img'], data_dict['target_img']])
        logger.debug(f"inputs_face shape: {inputs_face.shape}")

        with torch.no_grad():
            logger.debug("\nProcessing grid sampling:")
            ...
            logger.debug(f"eye_vector shape: {eye_vector.shape}")

            theta_targ = torch.cat([data_dict['target_theta'][:, :3, :], data_dict['target_theta'][:, :3, :]])
            logger.debug(f"theta_targ shape: {theta_targ.shape}")

            theta_ = torch.cat([theta, eye_vector], dim=1)
            theta_targ_ = torch.cat([theta_targ, eye_vector], dim=1)
            logger.debug(f"theta_ shape: {theta_.shape}")
            logger.debug(f"theta_targ_ shape: {theta_targ_.shape}")
            ...
            logger.debug(f"inv_theta_2d shape: {inv_theta_2d.shape}")

            scale = torch.zeros_like(inv_theta_2d)
            scale_full = torch.zeros_like(inv_theta_2d)
            logger.debug(f"scale tensor shape: {scale.shape}")
            ...
            logger.debug(f"After scaling:")
            logger.debug(f"  inv_theta_2d shape: {inv_theta_2d.shape}")
            logger.debug(f"  inv_theta_targ_2d shape: {inv_theta_targ_2d.shape}")
            ...
            logger.debug(f"align_warp shape: {align_warp.shape}")
            
            inputs_face_aligned = F.grid_sample(inputs_face.float(), align_warp.float())
            logger.debug(f"inputs_face_aligned shape: {inputs_face_aligned.shape}")
            ...
            logger.debug(f"align_warp_targ shape: {align_warp_targ.shape}")
            ...
            logger.debug(f"align_warp_full shape: {data_dict['align_warp_full'].shape}")

        logger.debug("\nComputing final embeddings:")
        pose_embed = self.net_face(inputs_face_aligned)[0]
        logger.debug(f"pose_embed shape: {pose_embed.shape}")
        
        data_dict['source_pose_embed'] = pose_embed[:n]
        data_dict['target_pose_embed'] = pose_embed[-t:]
        logger.debug(f"Final embed shapes:")
        logger.debug(f"  source_pose_embed: {data_dict['source_pose_embed'].shape}")
        logger.debug(f"  target_pose_embed: {data_dict['target_pose_embed'].shape}")
...
class ResNetWrapper(nn.Module):
    def __init__(self,
        ...
        if self.custom_w:
            self.net = ResNet18()
            checkpoint = torch.load('/media/2TB/EMOPortraits/repos/official/Resnet18/checkpoints/best_checkpoint.tar')
