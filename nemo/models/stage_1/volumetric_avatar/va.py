...
class ResizeModule(nn.Module):
    """Proper module for image resizing instead of lambda."""
    ...
    def forward(self, img):
        return F.interpolate(img, mode=self.mode, size=self.size, align_corners=False)

class DefaultGridSample(nn.Module):
    """Default grid sample implementation without lambda."""
    def __init__(self, padding_mode='reflection'):
        super().__init__()
        self.padding_mode = padding_mode
    
    def forward(self, inputs, grid):
        return F.grid_sample(inputs.float(), grid.float(), padding_mode=self.padding_mode)
...
class Model(nn.Module):
    ...
    def __init__(self, DELETE, training=True, rank=0, exp_dir=None):
        ...
        self.resize_d = ResizeModule(size=(224, 224))
        self.resize_u = ResizeModule(size=(256, 256))
    ...
    def init_networks(self, args, training):
        ...
        if args.warp_norm_grad:
            ...
            self.grid_sample = DefaultGridSample(padding_mode=self.args.grid_sample_padding_mode)
    ...
    def G_forward(self, data_dict, visualize, iteration=0, epoch=0):
        print("🥊:",data_dict)
        """Forward pass of the face animation generator with timing measurements."""
        timing_stats = TimingStats()
        stage_start = time.perf_counter()

        self.visualize = visualize
        b, c, s, d = self._get_batch_dimensions(data_dict)

        # Face Parsing & Masking
        tic = time.perf_counter()
        if self.args.use_mix_mask:
            data_dict = self._process_face_masks(data_dict)
        timing_stats.update('face_parsing', time.perf_counter() - tic)

        # Head Pose Estimation
        tic = time.perf_counter()
        if self.args.estimate_head_pose_from_keypoints:
            data_dict = self._compute_pose_warping_fields(data_dict, b, d, s)
        else:
            data_dict = self._compute_3dmm_warping_fields(data_dict, b, d, s)
        timing_stats.update('pose_estimation', time.perf_counter() - tic)

        # Feature Extraction
        tic = time.perf_counter()
        latent_volume = self._extract_features_and_embeddings(data_dict)
        embedding_results = self.predict_embed(data_dict)
        source_warp_embed_dict, target_warp_embed_dict, mixing_warp_embed_dict, embed_dict = embedding_results
        timing_stats.update('feature_extraction', time.perf_counter() - tic)

        # Warping Fields Generation
        tic = time.perf_counter()
        source_xy_warp_resize, target_uv_warp_resize = self._generate_warping_fields(
            source_warp_embed_dict, target_warp_embed_dict)
        data_dict.update({
            'source_xy_warp_resize': source_xy_warp_resize,
            'target_uv_warp_resize': target_uv_warp_resize
        })
        timing_stats.update('warp_generation', time.perf_counter() - tic)

        # Background Processing
        target_latent_feats_back = None
        if self.args.use_back:
            tic = time.perf_counter()
            target_latent_feats_back = self._process_background(data_dict, b, c, d, s)
            timing_stats.update('background_processing', time.perf_counter() - tic)

        # Volume Processing Pipeline
        tic = time.perf_counter()
        latent_volume = self._process_latent_volume(latent_volume, embed_dict, b, c, d, s)
        latent_volume = self._normalize_source_pose(latent_volume, data_dict)
        canonical_latent_volume = self._generate_canonical_volume(latent_volume, embed_dict)
        timing_stats.update('volume_processing', time.perf_counter() - tic)

        # Optional Tensor Addition
        if self.args.use_tensor:
            canonical_latent_volume += self.avarage_tensor_ts

        # Canonical Volume Supervision
        if self.args.predict_target_canon_vol and not (epoch == 0 and iteration < 0):
            tic = time.perf_counter()
            self._process_canonical_volume_supervision(data_dict, canonical_latent_volume, b, c, d, s)
            timing_stats.update('canonical_supervision', time.perf_counter() - tic)

        # Target Generation Pipeline
        tic = time.perf_counter()
        aligned_target_volume = self._generate_target_pose(
            canonical_latent_volume, 
            data_dict,
            target_latent_feats_back
        )
        data_dict['pred_target_img'], _, _, _ = self.decoder_nw(
            data_dict, target_warp_embed_dict, aligned_target_volume, False, iteration=iteration
        )
        timing_stats.update('target_generation', time.perf_counter() - tic)

        # Neutral Expression Processing
        if self.args.match_neutral:
            tic = time.perf_counter()
            self._process_neutral_expression(
                data_dict, canonical_latent_volume, target_warp_embed_dict,
                target_latent_feats_back, b, c, d, s, epoch, iteration
            )
            timing_stats.update('neutral_expression', time.perf_counter() - tic)

        # Final Masking
        tic = time.perf_counter()
        data_dict = self._apply_final_masks(data_dict)
        timing_stats.update('final_masking', time.perf_counter() - tic)

        # Expression Mixing
        if self.pred_mixing or not self.training:
            tic = time.perf_counter()
            data_dict = self._process_expression_mixing(
                data_dict, canonical_latent_volume, mixing_warp_embed_dict,
                b, c, d, s, epoch, iteration
            )
            timing_stats.update('expression_mixing', time.perf_counter() - tic)

        # Log timing statistics
        total_time = time.perf_counter() - stage_start
        timing_stats.update('total', total_time)
        
        if iteration % 100 == 0:  # Log every 100 iterations
            stats = timing_stats.get_stats()
            log_msg = f"\nTiming Stats (Iteration {iteration}):\n"
            log_msg += "\n".join([
                f"{stage}: {stats[stage]['avg_ms']:.2f}ms (called {stats[stage]['count']} times)"
    ...
    def _extract_features_and_embeddings(self, data_dict):
        """Extract latent features and embeddings from source image."""
        source_masked = data_dict['source_img'] * data_dict['source_mask']
        latent_volume = self.local_encoder_nw(source_masked)
    ...
    def predict_embed(self, data_dict):
        ...
        print(f"DEBUG: source_pose_embed shape: {data_dict['source_pose_embed'].shape if 'source_pose_embed' in data_dict else 'N/A'}")
        print(f"DEBUG: target_pose_embed shape: {data_dict['target_pose_embed'].shape if 'target_pose_embed' in data_dict else 'N/A'}")
        print(f"DEBUG: idt_embed shape: {data_dict['idt_embed'].shape}")
        for k, (pose_embed, m) in enumerate(zip(pose_embeds, num_frames)):
            print(f"DEBUG: Processing k={k}, pose_embed shape: {pose_embed.shape}, m={m}")


            if self.args.cat_em:
                idt_repeated = data_dict['idt_embed'].repeat_interleave(m, dim=0)
                print(f"DEBUG: cat_em=True, pose_embed shape: {pose_embed.shape}, idt_repeated shape: {idt_repeated.shape}")
                warp_embed_orig = self.warp_embed_head_orig_nw(torch.cat([pose_embed, idt_repeated], dim=1))
                warp_embed_orig_d = self.warp_embed_head_orig_nw(torch.cat([pose_embed.detach(), idt_repeated], dim=1))
            else:
                idt_repeated = data_dict['idt_embed'].repeat_interleave(m, dim=0)
                print(f"DEBUG: cat_em=False, pose_embed shape: {pose_embed.shape}, idt_repeated shape: {idt_repeated.shape}")
                print(f"DEBUG: Flattened shapes - pose: {pose_embed.view(pose_embed.shape[0], -1).shape}, idt: {idt_repeated.view(idt_repeated.shape[0], -1).shape}")
                warp_embed_orig = self.warp_embed_head_orig_nw((pose_embed + idt_repeated) * 0.5)
                warp_embed_orig_d = self.warp_embed_head_orig_nw((pose_embed.detach() + idt_repeated) * 0.5)
    ...
    ####################### Forward everithing ############################
    #######################################################################
    def forward(self, data_dict: dict, phase: str = 'test', optimizer_idx: int = 0,
            visualize: bool = False, ffhq_per_b=0, iteration=0, rank=-1, epoch=0):
        """
        Forward pass of the model handling both training and testing phases.
        ...
        Args:
            ...
            ffhq_per_b (int): FFHQ images per batch
            iteration (int): Current iteration number
            rank (int): Process rank for distributed training
            epoch (int): Current epoch number
        
        Returns:
            tuple: (loss, losses_dict, visuals, data_dict)
        """
        assert phase in ['train', 'test']
        print("foward:",data_dict)
        ...
        if mode == 'gen':
            loss, losses_dict = self._handle_generator_mode(data_dict, phase, resize, epoch, iteration, ffhq_per_b)
        elif mode == 'dis':
            loss, losses_dict = self._handle_discriminator_mode(data_dict, resize, epoch, ffhq_per_b)
        elif mode == 'dis_stylegan':
            loss, losses_dict = self._handle_stylegan_discriminator_mode(data_dict, iteration, epoch)
        else:
